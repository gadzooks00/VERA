function [annotation_remap,cmap,name,name_id] = createColormapFromAnnotations(surface,varargin)
    ...
    if(isempty(surface.AnnotationLabel) && ~isempty(surface.Annotation))
        ...
        orphan_idx = find(annotation_remap == 0);
        if ~isempty(orphan_idx)
            cmap(end+1,:) = [0 0 0];
            name{end+1}    = 'unlabelled';
            new = size(cmap,1);
            name_id(end+1) = new;
            annotation_remap(orphan_idx) = new; % these are
                % not associated with an annotation, so make a new entry in
                % cmap
        end
